...
static constexpr const char* MODULE = "SOLVER";

namespace {

std::vector<double> toml_array_to_vector(const toml::node & node)
{
  std::vector<double> result;
  if (auto * arr = node.as_array()) {
    for (auto & elem : *arr) {
      result.push_back(*elem.value<double>());
    }
  }
  return result;
}
...
Solver::Solver(const std::string & config_path) : R_gimbal2world_(Eigen::Matrix3d::Identity())
{
  ...
  auto R_gimbal2imubody_arr = config["solver"]["R_gimbal2imubody"];
  auto R_camera2gimbal_arr = config["solver"]["R_camera2gimbal"];
  auto t_camera2gimbal_arr = config["solver"]["t_camera2gimbal"];
  auto R_gimbal2imubody_data = toml_array_to_vector(*R_gimbal2imubody_arr.node());
  auto R_camera2gimbal_data = toml_array_to_vector(*R_camera2gimbal_arr.node());
  auto t_camera2gimbal_data = toml_array_to_vector(*t_camera2gimbal_arr.node());
  R_gimbal2imubody_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(R_gimbal2imubody_data.data());
  R_camera2gimbal_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(R_camera2gimbal_data.data());
  t_camera2gimbal_ = Eigen::Matrix<double, 3, 1>(t_camera2gimbal_data.data());

  auto camera_matrix_arr = config["solver"]["camera_matrix"];
  auto distort_coeffs_arr = config["solver"]["distort_coeffs"];
  auto camera_matrix_data = toml_array_to_vector(*camera_matrix_arr.node());
  auto distort_coeffs_data = toml_array_to_vector(*distort_coeffs_arr.node());
...
void Solver::optimize_yaw(Armor & armor) const
{
  ...
  auto yaw0 = tools::limit_rad(gimbal_ypr[0] - yaw_search_range_ / 2 * CV_PI / 180.0);
  auto range = static_cast<int>(yaw_search_range_);
  ...
  for (int i = 0; i < range; i++) {
    ...
    auto error = armor_reprojection_error(armor, yaw, (i - range / 2) * CV_PI / 180.0);
