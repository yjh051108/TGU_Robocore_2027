...
- [ ] **MATH-05**: 实现 `xyz2ypd_jacobian()` / `ypd2xyz_jacobian()` 雅可比矩阵
...
- [ ] **EKF-03**: 实现 NIS 卡方检验（recent_nis_failures）
...
- [ ] **CLS-01**: 实现基于灯条几何特征的装甲板类型判断（小装甲/大装甲）
- [ ] **CLS-02**: 实现基于名字的类型校验（英雄/基地只能是大装甲）

### PnP 解算 (Solver)

- [ ] **SLV-01**: 定义大/小装甲板 3D 模型点（mm 单位）
- [ ] **SLV-02**: 实现 solvePnP 姿态解算（solvePnP_IPPE）
- [ ] **SLV-03**: 实现相机→云台→世界坐标系变换
- [ ] **SLV-04**: 实现 yaw 优化（重投影误差最小化）
- [ ] **SLV-05**: 支持云台姿态更新（set_R_gimbal2world）

### EKF 目标跟踪 (Target)

- [ ] **TGT-01**: 实现 11 维状态向量 EKF 目标初始化（x, y, z, yaw, radius 等）
- [ ] **TGT-02**: 实现状态预测（状态转移矩阵 + Piecewise White Noise Model）
...
- [ ] **TRK-03**: 实现目标更新（update_target，类型和ID匹配）
- [ ] **TRK-04**: 实现发散检测
- [ ] **TRK-05**: 实现收敛判断
...
- [ ] **AIM-01**: 实现瞄准点选择（choose_aim_point，锁定模式/小陀螺模式）
- [ ] **AIM-02**: 实现弹道迭代解算（最多 10 次迭代收敛）
...
- [ ] **BLD-01**: 配置 `app/CMakeLists.txt` 构建目标
- [ ] **BLD-02**: 配置顶层 CMakeLists.txt 启用 app 子目录
...
|-------------|-------|--------|
| MATH-01~07 | Phase 1 | Pending |
| EKF-01~03 | Phase 2 | Pending |
| TRAJ-01~02 | Phase 2 | Pending |
| DATA-01~04 | Phase 3 | Pending |
| DET-01~09 | Phase 4 | Pending |
| CLS-01~02 | Phase 4 | Pending |
| SLV-01~05 | Phase 5 | Pending |
| TGT-01~05 | Phase 6 | Pending |
| TRK-01~05 | Phase 6 | Pending |
| VOT-01 | Phase 6 | Pending |
| AIM-01~04 | Phase 7 | Pending |
| SHT-01~02 | Phase 7 | Pending |
| CFG-01 | Phase 8 | Pending |
| BLD-01~02 | Phase 8 | Pending |
